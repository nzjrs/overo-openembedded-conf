...
echo cp $DL_DIR/$MLO $KERNEL_DIR/MLO
echo cp $DL_DIR/$UBOOT $KERNEL_DIR/u-boot.bin
echo cp $DL_DIR/$KERNEL $KERNEL_DIR/uImage
...
if [ "$ky" == "Y" ] ; then
    echo "===== Downloading           ====="
    wget -c -P $DL_DIR $MLO_URL $ROOTFS_URL $UBOOT_URL $KERNEL_URL
fi
...
if [ "$ky" == "Y" ] ; then
    ...
    sudo tar xvjf $DL_DIR/$ROOTFS -C $ROOTFS_DIR > /tmp/extract.log
fi
...
if [ "$ubt" == "Y" ] ; then
    echo "===== COPYING U-BOOT              ====="
    cp $DL_DIR/$UBOOT $KERNEL_DIR/u-boot.bin
fi
...
if [ "$rfsy" == "Y" ] ; then
    echo "===== COPYING KERNEL              ====="
    cp $DL_DIR/$KERNEL $KERNEL_DIR/uImage
fi
